...
import type { SeasonTitleVM } from "./season-titles-card";
...
export function AddTitleDialog({
  ...
  onAdded: (title: SeasonTitleVM) => void;
}) {
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    setSaving(true);
    fetch(`/api/seasons/${seasonId}/titles`, {
      method: "POST",
      ...
      <DialogContent className="sm:max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          {selected === NEW_COMPETITION && (
            <>
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="new-competition-name">Nome da competição</Label>
                <Input
              ...
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="new-competition-trophy">URL do troféu (opcional)</Label>
                <ImageUrlInput id="new-competition-trophy" value={trophyUrl} onChange={setTrophyUrl} />
              </div>
            </>
          )}
